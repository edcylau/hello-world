...
function Overlay({ children, isOpen, setIsOpen }) {
  ...
  return (
    <motion.div
      ...
      className="fixed z-50 block bg-themegreen-dark text-white inset-0"
      initial="closed"
      variants={backgroundVariants}
    >
      <div className="flex flex-col h-full max-h-full justify-between">
        <div className="flex justify-between items-center max-w-9/10 mx-auto w-full">
          <div></div>
          <Link to="/">
          ...
          >
            <FaTimes className="h-8 w-auto fill-current text-white" />
          </motion.button>
        </div>
        <motion.div
